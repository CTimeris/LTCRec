...
class BaseTrainer(metaclass=ABCMeta):
    ...
    def clip_gradients(self, limit=0.5):
    ...
    def _update_dataloader_metrics(self, tqdm_dataloader, meter_set):
        description_metrics = (['NDCG@%d' % k for k in self.metric_ks[2:]]
                               ...
                               + ['HitRate@%d' % k for k in self.metric_ks[2:]]
                               + ['MRR@%d' % k for k in self.metric_ks[2:]])
        description = 'Eval: ' + ', '.join(s + ' {:.4f}' for s in description_metrics)
        description = description.replace('NDCG', 'N').replace('Recall', 'R').replace('HitRate', 'H').replace('MRR', 'M')
    ...
    def _create_loggers(self):
        ...
        for k in self.metric_ks:
            ...
            val_loggers.append(
                MetricGraphPrinter(key='HitRate@%d' % k, graph_name='HitRate@%d' % k, group_name='Validation', use_wandb=self.use_wandb))
        ...
        for k in self.metric_ks:
            ...
            test_loggers.append(
                MetricGraphPrinter(key='HitRate@%d' % k, graph_name='HitRate@%d' % k, group_name='Test', use_wandb=self.use_wandb))
